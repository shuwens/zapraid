...
#include <ctime>
#include <string>
enum class DebugOutPutLevel : int { NONE, ERROR, WARN, INFO, TRACE, ANY };
...
public:
  ...
  void setDebugLevel(int level) { debugLevel_ = (DebugOutPutLevel)level; }
  DebugOutPutLevel getDebugLevel() { return debugLevel_; }
...
#define DEBUG_LEVEL                                                            \
  DebugManager::getInstance().getDebugLevel() // DebugOutPutLevel::ERROR
// for color
#define RESET "\033[0m"
...
#define debug_warn(fmt, ...)                                                   \
  do {                                                                         \
    if (DEBUG_LEVEL >= DebugOutPutLevel::WARN)                                 \
      fprintf(stderr, YELLOW "[%s] %s:%d:%s(): " fmt RESET, getTime().c_str(), \
  ...
  do {                                                                         \
    if (DEBUG_LEVEL >= DebugOutPutLevel::WARN)                                 \
      fprintf(stderr, YELLOW "[%s] %s:%d:%s(): " fmt "\n" RESET,               \
              getTime().c_str(), __FILE__, __LINE__, __func__);                \
  } while (0)

#define debug_error(fmt, ...)                                                  \
  do {                                                                         \
    if (DEBUG_LEVEL >= DebugOutPutLevel::ERROR)                                \
      fprintf(stderr, BOLDRED "[%s] %s:%d:%s(): " fmt RESET,                   \
              getTime().c_str(), __FILE__, __LINE__, __func__, __VA_ARGS__);   \
  } while (0)
...
#define debug_info(fmt, ...)                                                   \
  do {                                                                         \
    if (DEBUG_LEVEL >= DebugOutPutLevel::INFO)                                 \
      fprintf(stderr, CYAN "[%s] %s:%d:%s(): " fmt RESET, getTime().c_str(),   \
              __FILE__, __LINE__, __func__, __VA_ARGS__);                      \
  } while (0)
...
#define debug_trace(fmt, ...)                                                  \
  do {                                                                         \
    if (DEBUG_LEVEL >= DebugOutPutLevel::TRACE)                                \
      fprintf(stderr, MAGENTA "[%s] %s:%d:%s(): " fmt RESET,                   \
              getTime().c_str(), __FILE__, __LINE__, __func__, __VA_ARGS__);   \
  } while (0)
